...
createcivi () {
  ...
  mysql -e 'GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER, LOCK TABLES, CREATE TEMPORARY TABLES, TRIGGER, CREATE ROUTINE, ALTER ROUTINE, REFERENCES ON `'$CLIENTNAME'_civi`.* TO '\'$CLIENTNAME\''@'\''localhost'\'' IDENTIFIED BY '\'$password\'';'	
#manually:
#create database ppsp_civi_dev;
#GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER, LOCK TABLES, CREATE TEMPORARY TABLES, TRIGGER, CREATE ROUTINE, ALTER ROUTINE, CREATE VIEW ON ``.* TO ''@'localhost' IDENTIFIED BY '';
}
...
delete () {
  ...
  cd $ROOTDIR
  rm -r $CLIENTNAME
  a2dissite `basename $APACHECONFIGFILE`
  service apache2 reload
}
